...
import {
  ...
  editDescription,
  loadSubPosts,
} from "redux/reducers/subPageReducer";
import { notify } from "redux/reducers/notificationReducer";
import SortTabBar from "components/layout/SortTabBar";
...
const SubPage = () => {
  ...
  const handleEditDescription = async () => {
    try {
      ...
      setEditOpen(false);
      dispatch(
        notify(`Updated description of your sub: c/${subredditName}`, "success")
      );
    } catch (err) {
      dispatch(notify(getErrorMsg(err), "error"));
    }
            ...
            <div>
              {subPage.posts.results.length !== 0 ? (
                ...
                <div className={classes.noPosts}>
                  ...
                  <Typography variant="h6" color="secondary">
                    Be the first one to post in c/{subredditName}!
                  </Typography>
                </div>
              )}
            </div>
